...
Sandia National Laboratories currently has many libraries that
...
scientific software produced and used by the national labs rely on message passing 
...
coprocessors such as Graphical Processing Units (GPUs). 
...
high thread counts, decreased memory per thread, and relatively small instruction
 ...
 comes to high performance computing. Since GPUs have a smaller instruction set, 
...
to run on a GPU are highly dependent on hardware architecture.  In
...
We plan on reengineering a tensor contraction library within Trilinos. This library is part of the Intrepid sub-package of Trilinos. The 
...
permits. First, if the team has finished, or has made significant progess
